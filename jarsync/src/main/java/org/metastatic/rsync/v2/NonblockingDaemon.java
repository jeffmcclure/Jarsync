            ...
            try
              {
                ServerSocket s = null;
                if (address != null)
                  s = SSLUtil.wrapServerSocket(new ServerSocket(port,
                      SSLUtil.BACKLOG, InetAddress.getByName(address)));
                    ...
                    if (key.isAcceptable())
                      {
                        ...
                        logger.info("connection made by " + host.getHostName()
                            + " (" + host.getHostAddress() + ") on port "
                            + c.socket().getPort());
                        if (statsMod != null)
                          {
